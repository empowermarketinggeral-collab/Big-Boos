...
import { c, sans, serif, Eyebrow, btnPrimary, btnGhost } from "../../shared/theme.jsx";
  ...
  return (
    <div className="bb-page" style={{ padding: "8px 40px 60px", maxWidth: 900 }}>
      ...
      <h1 style={{ ...serif, fontSize: 24, color: c.ink, margin: "0 0 16px", display: "flex", alignItems: "center", gap: 10 }}>
        <CreditCard size={20} color={c.boss} /> Subscrição da agência
      </h1>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
              {(plansQuery.data || []).map((plan) => (
                <div key={plan.id} style={{ border: `1px solid ${c.line}`, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ ...sans, fontSize: 18, fontWeight: 700, color: c.ink, marginBottom: 10 }}>
                    {plan.contact_sales ? <span style={{ fontSize: 15 }}>Personalizado</span> : <>{money(plan.price_cents, plan.currency)}<span style={{ fontSize: 11, fontWeight: 400, color: c.mist }}>/mês</span></>}
                  </div>
